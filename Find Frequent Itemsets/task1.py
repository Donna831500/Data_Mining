...
if case == 1:
    #basket = data.map(lambda x: x.split(',')).map(lambda x: (x[0], [x[1]])).reduceByKey(lambda x, y: x + y).collectAsMap()
    basket_dict = data.map(lambda x: x.split(',')).map(lambda x: (x[0], [x[1]])).reduceByKey(lambda x, y: x + y).filter(lambda x: x[0]!="user_id").map(lambda x:(x[0],list(set(x[1])))).collectAsMap()

else:
    basket_dict = data.map(lambda x: x.split(',')).map(lambda x: (x[1], [x[0]])).reduceByKey(lambda x, y: x + y).filter(lambda x: x[0]!="business_id").map(lambda x:(x[0],list(set(x[1])))).collectAsMap()
